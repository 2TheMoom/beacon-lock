...
```bash
# Linting
genvm-lint check contracts/beacon_lock.py       # Lint the contract
...
This is a contract-only primitive (no frontend) - see README.md for what it
does and how another builder would integrate it.

## Development Workflow

1. Write/modify contract in `contracts/`
2. Lint: `genvm-lint check contracts/beacon_lock.py`
3. Test direct: `pytest tests/direct/ -v`
4. Select a network and deploy: `genlayer network` then `genlayer deploy`
